...
namespace searchPackagesHintHack
{
  ...
  namespace
  {
	...
	bool _wasBlocked;
      ...
      return( it.findPackage( "zypper" ) && it->tag_provides().count( Capability("y2pmsh") ) );
    ...
    bool userDecidedToCallSearchPackages( Zypper & zypper_r )
    {
      zypper_r.out().notePar( "FAKE PROMPT: WILL FWD..." );
      return true;
    }
  ...
  void callOrNotify( Zypper & zypper_r )
  {
    ...
    // Here we're in 'search' AND >=SLE15-SP1 and may execute the subcommand....

    if ( zypper_r.runningShell() )
    {
      // In shell we'd need to drop everything and re-aquire it again after the
      // subcommand. While we don't support subcommands in shell, we don't run it here.
      printSearchPackagesHint( zypper_r, plgInstalled );
      return;
    }
    ...
    if ( userDecidedToCallSearchPackages( zypper_r ) )
